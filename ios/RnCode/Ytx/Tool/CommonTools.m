...
+ (UIImage *)createImageWithColor:(UIColor *)color withSize:(CGSize)size text:(NSString*)text
{
    ...
    CGContextSetFillColorWithColor(context, [color CGColor]);
    CGContextFillRect(context, rect);
    CGFloat width = [text sizeWithFont:[UIFont systemFontOfSize:11.0f]].width;
    if ([UIDevice currentDevice].systemVersion.floatValue>7.0) {
        [text drawInRect:CGRectMake((size.width-width)/2,size.height*0.3, width, size.height*0.6) withAttributes:@{NSFontAttributeName :[UIFont systemFontOfSize:11.0f], NSForegroundColorAttributeName :[ UIColor whiteColor]}];
    }
    UIImage *theImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
...
+ (UIImage *)changeImage:(UIImage*)img WithColor:(UIColor *)color
{
    ...
    UIGraphicsBeginImageContextWithOptions(imageRect.size, NO, img.scale);
...
+ (NSString*)stringFromDate:(NSString*)dateformateStr WithDate:(NSDate*)date{
    NSDateFormatter *datefmt = [[NSDateFormatter alloc] init];
    datefmt.dateFormat = dateformateStr;
    return [datefmt stringFromDate:date];
}

+ (NSDate*)dateFromString:(NSString*)dateformateStr WithString:(NSString*)timestr{
    NSDateFormatter *datefmt = [[NSDateFormatter alloc] init];
    datefmt.dateFormat = dateformateStr;
    return [datefmt dateFromString:timestr];
}
...
+ (NSString *)imei
{
    return [self MD5:[self macAddress]];
}
